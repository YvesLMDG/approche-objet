...
public class ComptagePaysParContinent {
	...
	public static void main(String[] args) {
		...
		Map<String, Integer> mapContinantHabs = new HashMap<>();
		
		for(Pays pays: listePays) {
			String continent = pays.continent;
			if (!mapContinantHabs.containsKey(continent)){
				mapContinantHabs.put(continent, 0);
			}
			mapContinantHabs.put(continent,mapContinantHabs.get(continent) + pays.nbHabs );
		}
		
		for(String continent: mapContinantHabs.keySet()) {
			System.out.println("Continent : " + continent + " nbHabs : " + mapContinantHabs.get(continent));
		}
	}
